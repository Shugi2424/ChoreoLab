import { roundCopScore } from "./scoring.js";

/** How additional turns beyond the minimum base are measured (CoP §12.2.5–12.2.8). */
export type PivotTurnUnit = "360" | "180";

export interface PivotRotationRule {
  turnUnit: PivotTurnUnit;
  /** Extra value per turn above the minimum base turn. `null` = no additional value. */
  incrementPerTurn: number | null;
  /** Human-readable hint for the rotation picker. */
  turnLabel: string;
}

const PIVOT_NO_ADDITIONAL_IDS = new Set(["3.2101", "3.2202"]);

/** CoP §12.10 — back split without help; +0.10 per extra 360° regardless of base value. */
const PIVOT_BACK_SPLIT_WITHOUT_HELP_IDS = new Set(["3.1005"]);
const PIVOT_BACK_SPLIT_WITHOUT_HELP_PREFIXES = ["3.1105"];
...
function isBackSplitWithoutHelpPivot(elementId: string): boolean {
  return (
    PIVOT_BACK_SPLIT_WITHOUT_HELP_IDS.has(elementId) ||
    PIVOT_BACK_SPLIT_WITHOUT_HELP_PREFIXES.some((prefix) => elementId.startsWith(prefix))
  );
}
...
export function getPivotRotationRule(elementId: string, baseValue: number): PivotRotationRule {
  ...
  if (isBackSplitWithoutHelpPivot(elementId)) {
    return {
      turnUnit: "360",
...
export function calculatePivotValue(
  ...
  if (!Number.isInteger(turnCount) || turnCount < 1) {
    throw new Error("turnCount must be a positive integer");
  }
  ...
  return roundCopScore(baseValue + additionalTurns * rule.incrementPerTurn);
}
